...
void	select_option_wrapper(t_hbranch *hbranch, void *arg)
{
	...
	clicked_index = get_hbranch_index(hbranch->parent->childs, hbranch) + 1;
	if (clicked_index == select->option_index && select->nullable)
	{
		select->option_index = 0;
		ft_strlcpy(select->selected->content,
			get_hbranch_child_idx(select->label_box, 0)->textbox
	...
	else
	{
		...
		ft_strlcpy(select->selected->content, 
			...
			.content, SELECT_LABEL_LEN);
		switch_select_expand(hbranch->parent->parent->parent->parent, NULL);
		option_action = get_vector_value(select->actions,
				select->option_index - 1);
		(*option_action)(hbranch, arg);
	}
}

static inline t_hbranch	*add_button_label(t_hbranch *select,
						t_hbranch *option, char option_name[SELECT_LABEL_LEN])
{
	t_hbranch	*option_label;

	option_label = add_textbox(option, (t_text){.font_size = 2,
...
static inline t_hbranch	*add_button_option(t_hbranch *select,
						char option_name[SELECT_LABEL_LEN])
{
	t_hbranch	*option;

	option = add_button(select->select.margin,
	...
	if (!add_button_label(select, option, option_name))
		return (NULL);
	return (option);
}
		...
		void (*option_action)(t_hbranch *hbranch, void *arg))
{
	...
	{
		ft_strlcpy(select->select.selected->content,
			option_name, SELECT_LABEL_LEN);
	...
	if (!add_button_option(select, option_name))
		return (1);
	return (0);
}
