...
static inline void	add_button_label(t_hbranch *select,
...
static inline void	add_button_option(t_hbranch *select,
						char option_name[SELECT_LABEL_LEN])
{
	...
	add_button_label(select, option, option_name);
}
		...
		void (*option_action)(t_hbranch *hbranch, void *arg))
{
	...
	if (vector_add(select->select.actions, &option_action, 1) != 0)
		return (1);
	add_button_option(select, option_name);
	return (0);
}
