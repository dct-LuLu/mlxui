...
static inline t_hbranch	*create_expand_label(t_hbranch *new, char label[SELECT_LABEL_LEN])
{
	t_hbranch	*label_box;
	t_hbranch	*label_text;

	label_box = add_box(new, (t_radius){.style = LOCAL_PX, .lt = 9, .rt = 9},
			(t_border){.size = 1, .color = new->head->style.border, .style = SOLID});
	...
	label_text = add_textbox(label_box,
			(t_text)
			{
				.font_size = 2,
...
static inline void	create_expand(t_hbranch *new, char label[SELECT_LABEL_LEN])
{
	...
	expand_box = add_box(label_box, (t_radius){.style = LOCAL_PX, .lb = 9, .rb = 9},
			(t_border){.size = 1, .color = new->head->style.border, .style = SOLID});
...
t_hbranch	*add_select(t_hbranch *parent_branch, char label[SELECT_LABEL_LEN])
{
	...
	new = add_button(parent_branch, (t_radius){.style = FULL_PX, .full = 9},
			(t_border){.size = 1, .color = parent_branch->head->style.border, .style = SOLID});
	new->type = SELECT;
