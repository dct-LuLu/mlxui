...
static inline bool	is_backspace_key(const int keycode) // to move to mlx_wrapper
{
...
{
	return (is_tkl_char_key(keycode) || is_numpad_char_key(keycode) || is_enter_key(keycode) || is_backspace_key(keycode));
}
...
static inline void	handle_typing_form(t_hbranch *hbranch, t_mlx *mlx_data)
{
	...
	{
		c = get_form_char_input(mlx_data->key_input.keycode, mlx_data->key_input.caps ^ mlx_data->key_input.shift);
...
void	hook_typing_form(t_hbranch *hbranch, t_mlx *mlx_data)
{
	...
	if (is_enter_key(mlx_data->key_input.keycode))
	{
		...
		//hbranch->form.format_buf(hbranch->form.edit_form_buf);
	...
	ft_strlcpy(hbranch->form.form_text->content, hbranch->form.edit_form_buf, FORM_BUF_SIZE);
	// et la taille qu'il reste sur le form (text width and str len)
}
