...
/*   Updated: 2025/12/03 19:45:27 by jaubry--         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "hierarchy_tree.h"

static inline void	create_button(t_hbranch *new, t_radius radius, t_border border)
{
...
t_hbranch	*add_button(t_hbranch *parent_branch, t_radius radius, t_border border)
{
	...
	add_func_button_hook(new->head->mlx_data, MLCLICK, (void (*)(t_vec2i, t_maction, void *, t_mlx *))hook_click_button, new);
	add_func_move_hook(new->head->mlx_data, (void (*)(void *, t_mlx *))hook_hover_button, new);
	return (new);
}
