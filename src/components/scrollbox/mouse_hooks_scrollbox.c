...
/*   Updated: 2025/12/23 04:00:20 by jaubry--         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	hbranch->scrollbox.scrollbar->pos.y = hbranch->_rt.y + ((float)hbranch->scrollbox._current_pos / (float)hbranch->scrollbox.inside->size.y) * (float)hbranch->size.y;
...
void	hook_scrollup_scrollbox(t_vec2i pos, t_maction action, t_hbranch *hbranch, t_mlx *mlx_data)
{
	...
	if ((action == MPRESS) && hbranch->visible && is_inside_comp(hbranch, pos))
	{
		...
		if (hbranch->scrollbox._current_pos > (hbranch->scrollbox._scroll_buffer.height - hbranch->size.y))
			hbranch->scrollbox._current_pos = hbranch->scrollbox._scroll_buffer.height - hbranch->size.y;
