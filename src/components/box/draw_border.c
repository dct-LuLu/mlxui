...
/*   Updated: 2025/10/30 18:48:42 by jaubry--         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_border(t_img_data *img, t_box box)
{
	...
	else
	{
		...
		{
			ft_mlx_hline_aput(img, vec2i(box._ltc.horz.x, box._rtc.horz.x).data, box._ltc.corner.y, box.border.color);
			ft_mlx_hline_aput(img, vec2i(box._lbc.horz.x, box._rbc.horz.x).data, box._lbc.corner.y, box.border.color);
			ft_mlx_vline_aput(img, vec2i(box._ltc.vert.y, box._lbc.vert.y).data, box._ltc.corner.x, box.border.color);
			ft_mlx_vline_aput(img, vec2i(box._rtc.vert.y, box._rbc.vert.y).data, box._rtc.corner.x, box.border.color);
		}
